...
AbstractOverlayWidget {
	...
	property string title: identifier.replace(/([A-Z])/g, " $1").replace(/^./, function (str) {
	...
	cursorShape: {
		if (dragHandler.active)
			return root.resizing ? cursorShape : Qt.ArrowCursor;
		if (resizeMargin < mouseX && mouseX < width - resizeMargin && resizeMargin < mouseY && mouseY < height - resizeMargin) {
	...
	onPressed: event => {
		...
		if (!root.resizable)
			return;
		if (root.resizeMargin < event.x && event.x < root.width - root.resizeMargin && root.resizeMargin < event.y && event.y < root.height - root.resizeMargin) {
			return;
		}
		// Resizing setup
		root.resizing = true;
