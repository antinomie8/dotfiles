...
Singleton {
	...
	function toggleRunning(running) {
		if (recognizeMusicProc.running && !running === true)
	...
	Process {
		...
		running: false
		command: ["notify-send", Translation.tr("Music Recognized"), root.recognizedTrack.title + " - " + root.recognizedTrack.subtitle, "-A", "Shazam", "-A", "YouTube", "-a", "Shell"]
		stdout: StdioCollector {
			onStreamFinished: {
				if (this.text === "")
					return;
